...
''' convert genetab format to standard genebed format '''
    ...
    with open(sys.argv[1],'r') as fh:
        for line in fh:
            if not line.startswith("#"):
                ...
                g=genetab(line)
                print '%s\t%d\t%d\t%s\t%s\t%s\t%d\t%d\t%s\t%d\t%s,\t%s,' %(g.chr, g.start, g.stop, g.id, g.score, g.strand, g.cds_start, g.cds_stop, '0', g.exon_count, ','.join(map(str,g.exonSizes)), ','.join(map(str,g.exonStartsR))  )

if __name__== "__main__":
    if len(sys.argv) == 1:
        print >>sys.stderr, "python Genebed2Genetab.py genetab(genePred).file"
    else:
        main()
