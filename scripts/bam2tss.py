...
def show_help():
    print >>sys.stderr,"bam2tss.py:  count reads in TSS region (default: -1000,+1000)"
    print >>sys.stderr,"Version:"+Version+"\n"
    print >>sys.stderr,"Library dependency: pysam\n\n"
    print >>sys.stderr,"Usage: bam2tss.py <options> --gene gene.tab file1.bam file2.bam > output.tab"
    print >>sys.stderr,"       bam2tss.py <options> --bed gene.bed --bam bamfiles > output.tab"
    print >>sys.stderr,"Example:  bam2tss.py --gene hg19.refseq.tab -b binsize file.bam > output.tab"
    print >>sys.stderr,"          bam2tss.py --bam bamlistfile --bed ncRNA.bed > output.tab"
...
def BamToTSS(bamfilename,norm=1000000):
...
def Main():
    global Table,Version,Binsize,ifRegion,Region,ifNormalize,File_reads_number,TssList,bamlist,Norm
    Version="0.1"
    Table={}
    if len(sys.argv)<2:
        show_help()
        exit(0)
    opts,restlist = getopt(sys.argv[1:],"hb:g:nu",\
                        ["help","binsize=","bam=","normalize","gene=","uniq","bed="])
    Binsize=2000
    ifNormalize=False
    File_reads_number={}
    ifBamFileList=0
