...
# Programmer : zhuxp
# Date: 
# Last-modified: 01-04-2014, 21:15:13 EST
VERSION="0.3"
'''
...
def ParseArg():
    ...
    p.add_argument('-p','--processor',dest="processor",type=int,default=4,help="processor number: DEFAULT %(default)i")
    if len(sys.argv)==1:
...
def Main():
    ...
    for i in range(args.processor):
        dbi.append(DBI.init(args.db,args.dbformat,**init_dict))
    if args.input=="stdin":
        input=sys.stdin
    else:
        input=args.input
...
def processing(i,lock,dbi_local,list_local):
    for x in list_local:
        retv=query(dbi_local,x)
        lock.acquire()
        print >>out,"# PROCESSOR NO."+str(i)+" REPORT"
        print >>out,retv
        print >>out,"\n"
        lock.release()
