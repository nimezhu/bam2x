...
# Programmer : zhuxp
# Date: 
# Last-modified: 02-04-2014, 14:58:16 EST
VERSION="pv20h"
...
def query(i,dbi_bam,genome): # i is query iteem
    ...
    for j,x in enumerate(cliques):
        ...
        if float(uniq_score)/total_frag < args.min_uniq :
            ...
            pattern[-1]=True
            pattern[-2]=True
            beds=[bed for bed in translate_bits_into_beds(g,cliques_pattern_no_prior[j])]
            #print "debug,bed",bed
            '''
            ...
            for j1,bed in enumerate(beds):
                bed.score=score*1000.0/cdna_length
                bed.chr=i[NAME]
                bed.id=i[NAME]+"_"+"NO."+str(j0)+"_"+str(j1)
                bed.itemRgb=str(rgb)+","+str(rgb)+","+str(rgb)
                ret_dict["CLIQUES"][-1]["BEDS"].append(str(Tools.translate_coordinates(Bed(i),bed,True)))
