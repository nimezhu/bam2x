...
in bamlist
all bam file should have same references.
this version is updated for paired end data.
...
#        print >>sys.stderr,"debug",i
    ...
    reads_set=[]
    reads_num=0
    for i in dbi.query(transcript_region,method="paired_end"):
        reads_set.append(i)
        reads_num+=1

    print >>out,"FRAGMENTS_NUMBER\t",reads_num
    ...
    for i in reads_set:
        bincode=0
        for j0,j in enumerate(isoforms_set):
            k=Tools.compatible_with_transcript(i,j,references=dbi.bamfiles[0].references,strand=args.strand)
