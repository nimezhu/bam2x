...
# Programmer : zhuxp
# Date: 
# Last-modified: 03 Oct 2012 16:55:32
...
class BamBins:
    ...
    def iterPeaks(self):
        ...
        for i in self.parse_segment():
            ...
            yield Peak([chr,refine_start,refine_stop,reads_num,pvalue,coverage,peak_pos,peak_intensity])
...
def ParseArg():
    ...
    p.add_argument('--elongation','-E',type=str,dest='eBamfile',required=True,help="the bamfile of H3K36me3")
    p.add_argument('--promoter','-P',type=str,dest='pBamfile',required=True,help="the bamfile of H3K4me3 (or H3K27ac etc.)")
    p.add_argument('--control','-C',type=str,dest='cBamfile',help="the bamfile of IgG (INPUT)",default=None)
...
def iterNearPromoter(chrom,start,stop):
    ...
    for bin in iterRangeOverlapBins(near_start,near_stop):
        for p in pTable[chrom][bin]:
                if p.start < near_stop and p.stop > near_start: 
                    yield p
...
def Main():
    ...
    if args.cBamfile is not None:
        cbambin=BamBins(args.cBamfile,args.Binsize)
        cbambin.callBinPeak(args.Pvalue)
    else:
        cbambin=None
    
    ebambin=BamBins(args.eBamfile,args.Binsize)
    ebambin.process(args.Pvalue,cbambin)
    pbambin=BamBins(args.pBamfile,args.Binsize)
    pbambin.process(args.Pvalue,cbambin)
    ...
    for i in ebambin.iterPeaks():
        (chrom,start,stop,reads_num,pvalue,coverage,peak_pos,peak_i)=(i.chr,i.start,i.stop,i.reads_num,i.pvalue,i.coverage,i.peak_pos,i.peak_coverage)
        k+=1
        t=0
        plus=0
        plus_reads=0
        ...
        for p in iterNearPromoter(chrom,start,stop):
                if p.start<start+thres and p.stop < stop-thres: 
                    plus+=1
                    plus_reads+=p.reads_num 
                if p.stop>stop-thres and p.start > start+thres: 
                    minus+=1
                    minus_reads+=p.reads_num
                t+=1
        gene=[]
        if args.genetab:
        ...
        if len(gene)==0 and len(fs)==0 and t>0:
            ...
            KorN="NP"
        print >>out,KorN,"\t",
        print >>out,"Transcript_"+str(k),"\t",i.tab(),
        if t==0:
            print >>out,"\t","None"
        ...
        for p in iterNearPromoter(chrom,start,stop):
            ...
            if p.start < start+thres and p.stop < stop-thres:
                print >>out,"+"
            elif p.start > start+thres and p.stop > stop-thres:
                print >>out,"-"
            else:
                print >>out,"."
        print >>out,"//\n"
